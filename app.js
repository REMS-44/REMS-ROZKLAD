...
const meta={
  ...
  conflicts:["Накладки","Усі конфлікти розкладу в одному місці"],
...
function openPlannerActionModal(html,wide=false){
  ...
  const card=layer.querySelector(".planner-action-card");
  card.classList.toggle("wide",!!wide);
  card.classList.toggle("student-load-mode",!!layer.querySelector(".student-load-popup"));
...
function closePlannerActionModal(){
  ...
  layer.querySelector(".planner-action-card")?.classList.remove("student-load-mode");
...
function occupancyGridRooms(){
  // The occupancy grid is a departmental working grid: only rooms explicitly
  // marked "show in grid" for the active programme are columns.
  // Schedule events from other programmes are still rendered inside these
  // shared rooms by roomEvents(), so conflicts remain visible without
  // flooding the grid with every room used anywhere in the approved timetable.
  return gridRooms();
}
...
function bookingConflicts(item,ignoreId=null){
  const conflicts=[];
  db.schedule.forEach(x=>{if(x.date!==item.date)return;const sameSlot=item.start&&item.end&&x.start&&x.end?timeOverlap(item.start,item.end,x.start,x.end):(item.pairId&&x.pairId?String(item.pairId)===String(x.pairId):false);if(!sameSlot)return;if(physicalRoomSame(item.room,x.room))conflicts.push(`Аудиторія ${item.room} вже зайнята заняттям ${x.group||""} ${x.discipline||""}.`);if(item.group&&x.group===item.group)conflicts.push(`Група ${item.group} уже має заняття.`);if(item.teacherId&&Number(x.teacherId)===Number(item.teacherId))conflicts.push(`Викладач ${item.teacher||""} уже має заняття.`);});
  db.roomBookings.forEach(x=>{if(Number(x.id)===Number(ignoreId)||x.date!==item.date)return;const sameSlot=item.start&&item.end&&x.start&&x.end?timeOverlap(item.start,item.end,x.start,x.end):(item.pairId&&x.pairId?String(item.pairId)===String(x.pairId):false);if(!sameSlot)return;if(physicalRoomSame(item.room,x.room))conflicts.push(`Аудиторія ${item.room} вже заброньована: ${roomBookingLabel(x)}.`);if(item.group&&x.group===item.group)conflicts.push(`Для групи ${item.group} уже є бронювання.`);if(item.teacherId&&Number(x.teacherId)===Number(item.teacherId))conflicts.push(`У викладача ${item.teacher||""} уже є бронювання.`);});
...
function globalConflictSameSlot(a,b){
  if(!a||!b||a.date!==b.date)return false;
  if(a.start&&a.end&&b.start&&b.end)return timeOverlap(a.start,a.end,b.start,b.end);
  if(a.pairId&&b.pairId)return String(a.pairId)===String(b.pairId);
  return false;
}
function globalConflictTeacherId(x){
  return Number(x?.sourceType==="booking"?x.teacherId:resolvedScheduleTeacherId(x,db))||null;
}
function globalConflictEventKey(x){return `${x.sourceType||"lesson"}:${x.id}`;}
function globalConflictIsSharedLesson(a,b){
  if(a.sourceType==="booking"||b.sourceType==="booking"||a.specialSchedule||b.specialSchedule)return false;
  const ta=globalConflictTeacherId(a),tb=globalConflictTeacherId(b);
  return !!(ta&&tb&&ta===tb
    && normIdentity(a.discipline||"")===normIdentity(b.discipline||"")
    && normIdentity(a.room||"")===normIdentity(b.room||"")
    && normIdentity(a.type||"")===normIdentity(b.type||"")
    && globalConflictSameSlot(a,b));
}
function globalConflictAudienceLabel(x){
  if(x.sourceType==="booking")return x.group||"—";
  return scheduleAudienceLabel(x)||specialStudentName(x)||x.group||"—";
}
function globalConflictDisciplineLabel(x){return x.discipline||x.title||x.kind||"Бронювання";}
function globalConflictTeacherLabel(x){
  if(x.teacher)return x.teacher;
  const tid=globalConflictTeacherId(x);
  return tid?teacherDisplay(teacherById(tid)):"—";
}
function globalConflictOpenAction(x){
  if(x.sourceType==="booking")return `openRoomBookingModal(${Number(x.id)})`;
  if(x.specialSchedule)return `openSpecialEventFromRoom(${Number(x.id)})`;
...
function conflictKindLabel(kinds){
  const labels=[];
  if(kinds.includes("room"))labels.push("аудиторія");
...
function conflictPairLabel(a,b){
  ...
  const ta=[a.start,a.end].filter(Boolean).join("–"),tb=[b.start,b.end].filter(Boolean).join("–");
  return ta===tb?ta:[ta,tb].filter(Boolean).join(" / ");
}
function conflictEventSummary(x){
  return `<div class="conflict-event-summary"><b>${esc(globalConflictAudienceLabel(x))}</b><span>${esc(globalConflictDisciplineLabel(x))}</span><small>${esc(globalConflictTeacherLabel(x))}${x.room?` · ауд. ${esc(x.room)}`:""}</small></div>`;
}
function updateConflictNavBadge(){
  const el=$("#conflictNavCount");if(!el)return;
  const n=collectGlobalConflicts().length;el.textContent=n;el.style.display=n?"":"none";
}
function renderConflicts(){
  const root=$("#page-conflicts");if(!root)return;
  const all=collectGlobalConflicts();
  const byType={room:all.filter(x=>x.kinds.includes("room")).length,teacher:all.filter(x=>x.kinds.includes("teacher")).length,audience:all.filter(x=>x.kinds.includes("audience")).length};
  const month=(window.conflictDashboardMonth||"");
  const type=(window.conflictDashboardType||"all");
  const filtered=all.filter(c=>(!month||c.date.startsWith(month))&&(type==="all"||c.kinds.includes(type)));
  root.innerHTML=`<div class="card section conflict-dashboard">
...
function explicitlyAllocatedTeacherIds(d){
  const ids=new Set([
    ...Object.keys(d?.teacherLoads||{}),
    ...Object.keys(d?.teacherStudentLoads||{}),
    ...Object.keys(d?.teacherStudentHours||{})
  ]);
  return [...ids].map(Number).filter(tid=>{
    if(!teacherById(tid))return false;
    const hasLoad=Object.values(d?.teacherLoads?.[String(tid)]||d?.teacherLoads?.[tid]||{}).some(v=>num(v)>0);
    const hasStudents=Object.values(d?.teacherStudentLoads?.[String(tid)]||d?.teacherStudentLoads?.[tid]||{}).some(ids=>Array.isArray(ids)&&ids.length>0);
    const hasStudentHours=Object.values(d?.teacherStudentHours?.[String(tid)]||d?.teacherStudentHours?.[tid]||{}).some(byStudent=>Object.values(byStudent||{}).some(h=>num(h)>0));
    return hasLoad||hasStudents||hasStudentHours;
  });
}
...
function splitIndividualAllocationPickerHtml(d,tid,lt){
  ...
  return `<div class="student-hour-picker">${studentsForDiscipline(d).map(s=>{
    ...
    return `<div class="student-hour-option ${own>0?"active":""} ${checked?"bulk-selected":""}" data-split-student="${s.id}">
...
function toggleSplitIndividualBulkStudent(tid,typeId,studentId,checked){
  ...
  const row=$(`[data-split-student="${sid}"]`);
...
function selectSplitIndividualBulkStudents(tid,typeId,on=true){
  ...
  $$('#studentLoadPicker [data-split-student]').forEach(row=>{
    ...
    const cb=row.querySelector('.student-hour-select input[type="checkbox"]');if(cb)cb.checked=checked;
  });
  const counter=$("#splitBulkSelectedCount");if(counter)counter.textContent=selected.size;
}
...
function openPerStudentAllocationPopup(tid,typeId){
  ...
  openPlannerActionModal(`<div class="student-load-popup">
    ...
    <div class="student-load-popup-tools">
      <div><b id="studentLoadSelectedCount">${s.count}</b><span>${s.split?"мають години":"обрано"} з ${s.groupCount}</span></div>
      <div><b id="studentLoadTotalHours">${fmtHours(s.total)} год</b><span>навантаження викладача</span></div>
      <button type="button" class="secondary" onclick="selectAllAvailablePerStudent(${tid},${typeId})">${s.split?"Віддати весь вільний залишок":"Обрати всіх вільних"}</button>
    ...
    <div class="planner-popup-footer">
      <button type="button" class="primary" onclick="${window.__directStudentAssignment?`saveDirectStudentAssignment(${tid},${typeId})`:`applyPerStudentAllocation(${tid},${typeId})`}">
        ${window.__directStudentAssignment?"Зберегти розподіл":"Зберегти розподіл"}
      </button>
    </div>
  </div>`,true);
}
function refreshPerStudentPopup(tid,typeId){
  ...
  if(!d||!lt)return;
  const picker=$("#studentLoadPicker"),oldScroll=picker?.scrollTop||0;
  const s=perStudentAllocationStatus(d,tid,lt);
  if(picker){picker.innerHTML=perStudentAllocationPickerHtml(d,tid,lt);picker.scrollTop=oldScroll;}
...
function persistPerStudentAllocationType(d,tid,typeId){
  if(!d?.id)return false;
  syncPerStudentAllocationLoads(d);
  const tkey=String(tid),lkey=String(typeId),hours=num(disciplineAllocationDraft?.[tkey]?.[lkey]);
  d.teacherLoads=d.teacherLoads||{};d.teacherLoads[tkey]=d.teacherLoads[tkey]||{};d.teacherLoads[tkey][lkey]=hours;
  d.teacherStudentLoads=d.teacherStudentLoads||{};d.teacherStudentLoads[tkey]=d.teacherStudentLoads[tkey]||{};
  d.teacherStudentLoads[tkey][lkey]=[...new Set((disciplineStudentAllocationDraft?.[tkey]?.[lkey]||[]).map(Number).filter(Boolean))];
  const lt=lessonTypeById(typeId);
  if(isSplitIndividualType(lt)){
    d.teacherStudentHours=d.teacherStudentHours||{};d.teacherStudentHours[tkey]=d.teacherStudentHours[tkey]||{};
    const clean={};Object.entries(disciplineStudentHoursDraft?.[tkey]?.[lkey]||{}).forEach(([sid,h])=>{if(num(h)>0)clean[String(sid)]=num(h);});
    d.teacherStudentHours[tkey][lkey]=clean;
  }
  const hasAny=Object.values(d.teacherLoads[tkey]||{}).some(v=>num(v)>0);
  d.teacherIds=[...new Set((d.teacherIds||[]).map(Number).filter(Boolean).concat(hasAny?[Number(tid)]:[]))];
  if(!hasAny)d.teacherIds=d.teacherIds.filter(id=>Number(id)!==Number(tid));
  save();
  return true;
}
function applyPerStudentAllocation(tid,typeId){
  ...
  if(!d)return;
  syncPerStudentAllocationLoads(d);
  persistPerStudentAllocationType(d,tid,typeId);
  closePlannerActionModal();
...
function specialLoadCardHtml(x){
  ...
  return `<article class="special-load-card ${done?"done":""} ${!resolved?"needs-students":""}" style="${scheduleColorVars({group:x.d.group,discipline:x.d.name})}">
    ...
    ${!resolved
      ...
      :`<div class="special-load-actions"><button class="${done?"secondary":"primary"}" onclick="openSpecialScheduleModal(${x.d.id},${x.t.id},${x.lt.id})" ${done?"disabled":""}>${done?"Навантаження вичерпано":"Додати годину студенту"}</button>${perStudent?`<button class="secondary" onclick="beginDirectStudentAssignment(${x.d.id},${x.t.id},${x.lt.id})">${split?"Змінити розподіл годин":"Змінити студентів"}</button>`:""}</div>`}
  </article>`;
}
  ...
  return `<div class="special-student-picker">${specialStudentsForLoad(d,t,lt).map(s=>{
    ...
    return `<button type="button"
      ...
      onclick="selectSpecialStudent(${s.id})">
        ...
        ${isPerStudentTypeId(lt.id)?`<small>цьому викладачу ${fmtHours(p.plan)} год · виставлено ${fmtHours(p.used)} · ${p.done?"ГОТОВО":`залишилось ${fmtHours(p.remaining)}`}</small>`:""}
    </button>`;
  }).join("")}</div>`;
}
...
function teacherTypePlan(d,teacherId,typeName){
  ...
  const lt=lessonTypeById(id);

  // Для поділених індивідуальних годин беремо фактичну частку викладача.
  if(isSplitIndividualType(lt)&&studentHourKeyExists(d,teacherId,id)){
    const actual=totalTeacherStudentHours(d,teacherId,id);
    const legacy=num(explicitTeacherLoad(d,teacherId)?.[id]);
    return actual>0||legacy<=0?actual:legacy;
  }
...
function scheduledLoad(disciplineId,teacherId,typeName,ignoreId=null){
  const ignored=new Set((Array.isArray(ignoreId)?ignoreId:(ignoreId==null?[]:[ignoreId])).map(Number));
  return db.schedule.filter(s=>!ignored.has(Number(s.id))&&scheduleCoversDiscipline(s,disciplineId)&&Number(s.teacherId)===Number(teacherId)&&s.type===typeName).reduce((a,s)=>a+num(s.workloadHours),0);
}
...
function setReadyScheduleMode(mode,existing=null){
  ...
  initRecurringEditScope(existing,"ready");
...
function readySameExternalTeacherConflict(item,ignoreId=null,extra=[]){
  ...
  return db.schedule.concat(extra||[]).filter(x=>{
    if(ignored.has(Number(x.id))||x.date!==item.date)return false;
    const sameSlot=item.start&&item.end&&x.start&&x.end
      ? timeOverlap(item.start,item.end,x.start,x.end)
      : (item.pairId&&x.pairId?String(item.pairId)===String(x.pairId):false);
...
function openReadyScheduleModal(editId=null){
  ...
  openModal(`<div class="ready-schedule-modal">
    ...
    <div class="ready-common-grid">
      ...
      ${existing?recurringEditScopeHtml(existing,"ready"):""}
    </div>
...
function saveReadySchedule(e,editId=null){
  ...
  const targetIds=editId?recurringEditTargetIds(existing,"ready"):[];
  const targetRows=editId?targetIds.map(id=>db.schedule.find(x=>Number(x.id)===Number(id))).filter(Boolean):[];
  ...
  rows.forEach((row,i)=>{
    ...
    const ignore=editId?targetIds:null;
    const cs=conflictsFor(item,ignore,draft);
    const textTeacher=readySameExternalTeacherConflict(item,editId?targetIds:null,draft);
  ...
  if(editId){
    if(!targetRows.length)return alert("Запис не знайдено.");
    const base=draft[0];
    const multi=targetRows.length>1;
    targetRows.forEach(target=>{
      const next={...base,date:multi?target.date:base.date,repeatBatchId:target.repeatBatchId||base.repeatBatchId};
      Object.assign(target,next,{id:target.id});
    });
  }else{
...
function teacherAvailabilityInfo(item,ignoreId=null,extra=[]){
  ...
  const existing=db.schedule
    .concat(extra||[])
    .filter(x=>!ignored.has(Number(x.id))&&Number(resolvedScheduleTeacherId(x,db))===Number(t.id)&&x.date===item.date);
...
function recurringEditTargetIds(existing,prefix){
  if(!existing)return [];
  const peers=recurringEditPeers(existing);
  if(peers.length<2)return [Number(existing.id)];
  const scope=$("#"+prefix+"EditScope")?.value||"one";
  if(scope==="one")return [Number(existing.id)];
  if(scope==="all")return peers.map(x=>Number(x.id));
  const ids=[...document.querySelectorAll(`[data-${prefix}-edit-date]:checked`)].map(el=>Number(el.value));
  return ids.length?ids:[Number(existing.id)];
}
function initRecurringEditScope(existing,prefix){
  if(!existing)return;
  const sel=$("#"+prefix+"EditScope"),box=$("#"+prefix+"EditDates");
  if(sel&&box)sel.onchange=()=>{box.style.display=sel.value==="selected"?"grid":"none";};
}
...
function openLessonModal(id=null,preset={}){
  ...
  openModal(`<h2>${id?"Редагувати заняття":"Додати заняття"}</h2><form id="lf" class="form-grid">
    ...
    <label class="wide">Примітка<input id="ln" value="${esc(x.note||"")}"></label>${existing?recurringEditScopeHtml(existing,"lesson"):""}<div id="conflictBox" class="wide"></div><div class="wide entity-form-actions"><button class="primary">${id?"Зберегти":"Додати"}</button>${id?`<button type="button" class="danger entity-delete-btn" onclick="deleteLesson(${id})">Видалити заняття</button>`:""}</div>
  </form>`,true);
  initRecurringEditScope(existing,"lesson");
  ...
  const check=()=>{const item=readLesson(),ignore=id?recurringEditTargetIds(existing,"lesson"):id,cs=conflictsFor(item,ignore),info=teacherAvailabilityInfo(item,ignore);let html="";if(cs.length||info.warnings.length)html+=conflictDetailsHtml(item,cs,info.warnings);if(info.notes.length)html+=`<div class="notice">${info.notes.map(esc).join("<br>")}</div>`;$("#conflictBox").innerHTML=html;};
  ...
  $("#lf").onsubmit=e=>{
    e.preventDefault();
    const base=readLesson();
    if(!base.discipline)return alert("Вкажіть дисципліну.");
    const d0=base.disciplineId?disciplineById(base.disciplineId):null,b0=d0?semesterDateBounds(d0.semester):academicYearBounds();
    if(!dateInBounds(base.date,b0))return alert(`Дата має бути в межах ${d0?`${d0.semester} семестру`:`навчального року`}: ${academicDateMessage(b0)}.`);
    if(!base.pairId&&(!base.start||!base.end||base.end<=base.start))return alert("Оберіть пару або коректний час.");

    const targetIds=id?recurringEditTargetIds(existing,"lesson"):[];
    const targetRows=id?targetIds.map(tid=>db.schedule.find(s=>Number(s.id)===Number(tid))).filter(Boolean):[];
    const multi=id&&targetRows.length>1;
    const drafts=(id?targetRows:[null]).map(target=>{
      const item={...base};
      if(target&&multi)item.date=target.date;
      if(target)item.repeatBatchId=target.repeatBatchId||base.repeatBatchId||null;
      refreshScheduleAudienceMetadata(item);
      return {target,item};
    });

    const d=base.disciplineId?disciplineById(base.disciplineId):null;
    if(d){
      if(!base.teacherId)return alert("Потрібно вибрати викладача з розподіленого навантаження.");
      const rem=remainingLoad(d,base.teacherId,base.type,targetIds);
      const needed=drafts.reduce((sum,x)=>sum+num(x.item.workloadHours),0);
      if(needed>rem+0.0001)return alert(`Недостатньо розподілених годин. Доступно для цих змін: ${fmtHours(rem)} год, потрібно ${fmtHours(needed)} год.`);
    }

    const warnings=[];
    const staged=[];
    for(const {target,item} of drafts){
      const cs=conflictsFor(item,targetIds,staged),info=teacherAvailabilityInfo(item,targetIds,staged);
      if(cs.length||info.warnings.length)warnings.push(`${formatDate(item.date)} · ${pairDisplay(item)}: ${[...conflictReasonLines(item,cs),...info.warnings].join(" ")}`);
      staged.push(item);
    }
    if(warnings.length&&!confirm("Є конфлікти або обмеження:\n\n"+warnings.join("\n")+"\n\nВсе одно зберегти?"))return;

    if(id){
      drafts.forEach(({target,item})=>Object.assign(target,item,{id:target.id}));
    }else db.schedule.push({id:uid(db.schedule),...base});
